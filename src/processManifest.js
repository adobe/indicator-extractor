...
import { generateIndicatorSet, extractMetadata, formatMetadata } from './indicatorSet.js';
  ...
  try {
    ...
    if (jumbfData && jumbfData.length > 0) {
      ...
      try {
        ...
        if (asIndicatorSet) {
          c2paInfo.indicatorSet = await generateIndicatorSet(manStore, validationResult, fileBuffer);
        } else {
        ...
        if (asIndicatorSet) {
          c2paInfo.indicatorSet = await generateIndicatorSet(null, null, fileBuffer);
      ...
      formatMetadata(c2paInfo.indicatorSet.metadata);
    }
  } catch (error) {
