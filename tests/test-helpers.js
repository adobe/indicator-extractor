...
class TestHelpers {
  ...
  /**
   * Processes a file using the CLI (indicator set generated by default), validates the indicator set file,
   * and asserts its structure and contents.
   *
   * @async
   * @param {string} inputImage - The input image file path.
   * @param {string} outputFileName - The name of the basic output file (for reference only, not created by default).
  ...
  async processFileAndValidateWithIndicatorSet(inputImage, outputFileName, indicatorFileName, options = {}) {
    ...
    const result = this.runCLI(inputImage, this.testDir, []);

    // Check that indicator set file was created
    expect(await fs.pathExists(indicatorSetFile)).toBe(true);

    // Read and parse the indicator set output
    const indicatorSetData = await this.readOutputJSON(indicatorFileName);

    // For validation, we need some basic structure - use indicator set data
    this.assertIndicatorSetStructure(indicatorFileName, { c2pa: { hasManifestStore: indicatorSetData.manifests && indicatorSetData.manifests.length > 0, manifestCount: indicatorSetData.manifests ? indicatorSetData.manifests.length : 0 } });

    // Check CLI output mentions the output file
